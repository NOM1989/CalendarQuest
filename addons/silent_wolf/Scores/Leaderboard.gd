...
func _on_load_specific_pressed():
	specific = true
	clear_leaderboard()
	specific_name = $LineEdit.text
	list_index = 0
	...
	add_loading_scores_message()
	var sw_result = await SilentWolf.Scores.get_top_score_by_player(specific_name, 100).sw_top_player_score_complete
	if sw_result.top_score != {}:
		scores = [sw_result.top_score.s]
	else:
		scores = []
	hide_message()
	render_board(scores, local_scores)
...
func render_board(scores: Array, local_scores: Array) -> void:
	...
	if ld_name in SilentWolf.Scores.ldboard_config and is_default_leaderboard(SilentWolf.Scores.ldboard_config[ld_name]):
		...
		if scores.is_empty():
			...
			return
	if specific:
		add_item(specific_name, str(scores[0]))
	else:
...
func add_item(player_name: String, score_value: String) -> void:
	...
	if !specific:
		...
		var sw_result = await SilentWolf.Scores.get_score_position(int(score_value)).sw_get_position_complete
		print(sw_result)
		print(score_value)
		item.get_node("PlayerName").text = str(str(sw_result.position - 1)) + str(". ") + player_name
